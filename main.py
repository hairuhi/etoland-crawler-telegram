...
# ========= 모니터링 대상 (요청사항 반영) =========
# 지정된 "유머게시판-약후" 리스트(hgall.php)만 모니터링
HGALL_URL = "https://www.etoland.co.kr/bbs/hgall.php?bo_table=etohumor07&sca=%BE%E0%C8%C4"
TARGET_BOARD = "etohumor07"  # 보드 필터 안전장치

# ========= 상태/테스트 =========
SEEN_SET_FILE = os.getenv("SEEN_SET_FILE", "state/seen_ids.txt")  # etoland:<bo_table>:<wr_id>
...
SESSION.headers.update({
    "User-Agent": "Mozilla/5.0 (compatible; EtolandCrawler/only-yakhu/1.0; +https://github.com/your/repo)",
...
def fetch_hgall_yakhu_list() -> list[dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        link = absolutize(HGALL_URL, a["href"])
        key = (bo, wr)
    ...
    print(f"[debug] hgall(약후) list fetched: {len(res)} items")
    return res
...
def process():
    ...
    posts.sort(key=lambda x: x["wr_id"])  # 오래된 것부터 전송
    ...
    for p in to_send:
        ...
        media_urls = images + videos  # 사진 우선
        MAX_ITEMS = 10
